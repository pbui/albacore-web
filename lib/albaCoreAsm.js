...
class Assembler {
    constructor() {
        ...
        this.text_instructions = [
            ...
            {regex:     /^bz\s+(?<rb>r[0-9]+),\s*(?<imm8>.+)$/,
             assembler: this.assemble_bz.bind(this)},
    ...
    assemble_br(imm8) {
        imm8 = this.parse_operand(imm8, 2, true);
        return "a" + imm8 + "0";
    }
    ...
    parse_operand(operand, width = 1, relative = false) {
        ...
        if (operand in this.text_labels) {
            if (relative) {
                ...
                operand = this.text_labels[operand].toString();
            }
        }
        ...
        if (operand.startsWith("0x")) {
            return operand.slice(2).padStart(width, "0");
        } else {
        ...
        for (let number = 0; number < source_lines.length; number++) {
            ...
            if (source == '.data') {
                ...
                if (memory_segment == MEMORY_SEGMENTS.data) {
                    ...
                    if (source.match(/^.*:$/)) {
                        dangling_label = source;
                    } else {
                        if (dangling_label) {
                            source         = dangling_label + " " + source;
                            dangling_label = "";
                        }
